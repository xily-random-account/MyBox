...
export class ImportPrompt implements Prompt {
	...
	public readonly container: HTMLDivElement = div({class: "prompt noSelection", style: "width: 300px;"},
		...
		p({style: "text-align: left; margin: 0.5em 0;"},
			"MyBox songs can be exported and re-imported as .json files. You could also use other means to make .json files for MyBox as long as they follow the same structure.",
		),
		p({style: "text-align: left; margin: 0.5em 0;"},
	...
	private _parseMidiFile(buffer: ArrayBuffer): void {
						...
						case MidiEventType.metaAndSysex: {
							if (eventStatus == MidiEventType.meta) {
								...
								if (message == MidiMetaEventMessage.endOfTrack) {
									...
									// beats. MyBox doesn't support fractional beats in a bar.
		...
		// Convert each midi channel into a MyBox channel.
						...
						for (let bar: number = startBar; bar < endBar; bar++) {
							...
							if (noteStartPart < noteEndPart) {
								...
								if (currentBar != bar || pattern == null) {
									...
									// program in this channel, create a new MyBox instrument for it.
								...
								const initialMyBoxPitch: number = Math.round((shiftedHeldPitch + currentMidiInterval) / intervalScale);
								...
								const potentialPins: PotentialPin[] = [
									{part: 0, pitch: initialMyBoxPitch, size: firstPin.size, keyPitch: false, keySize: false}
								];
								...
								for (let part: number = noteStartPart + 1; part <= noteEndPart; part++) {
									...
									// MyBox can only add pins at whole number intervals and sizes. Detect places where
									...
									if (keyPitch || keySize || lastPart) {
										...
										if (addPin) {
											...
											note.pins.push(makeNotePin(toBePinned.pitch - initialMyBoxPitch, toBePinned.part, toBePinned.size));
								...
								note.pins.push(makeNotePin(lastToBePinned.pitch - initialMyBoxPitch, lastToBePinned.part, lastToBePinned.size));
								...
								// Build the note chord out of the current pitches, shifted into MyBox channelBasePitch relative values.
								note.pitches.length = 0;
		...
		// For better or for worse, MyBox has a more limited number of channels than Midi.
