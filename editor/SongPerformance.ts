...
export class SongPerformance {
    ...
    // NEW: simple audio players for song.audioTracks.
    private _audioTrackPlayers: HTMLAudioElement[] = [];
    ...
    public play(): void {
        ...
        // NEW: start audio tracks when the song plays.
        this._startAudioTracks();
    }
    ...
    public pause(): void {
        ...
        // NEW: stop audio tracks when the song pauses.
    ...
    private _onAnimationFrame = (): void => {
        window.requestAnimationFrame(this._onAnimationFrame);
        this._syncAudioTracksToPlayhead();
    ...
    private _syncAudioTracksToPlayhead(): void {
        if (!this._doc.prefs.playAudioWithPlayhead || this._audioTrackPlayers.length == 0) return;
        const songBeat: number = this._doc.synth.playhead * this._doc.song.beatsPerBar;
        const secondsPerBeat: number = 60 / this._doc.song.tempo;
        const isPlaying: boolean = this._doc.synth.playing;
        const tracks: any[] = (this._doc.song as any).audioTracks || [];
        for (let index: number = 0; index < this._audioTrackPlayers.length; index++) {
            const audio: HTMLAudioElement = this._audioTrackPlayers[index];
            const track: any = tracks[index];
            if (track == null || track.muted || songBeat < (Number(track.startBeat) || 0)) {
                audio.pause();
                continue;
            }
    ...
    // NEW: start audio tracks when the song plays.
    private _startAudioTracks(): void {
        this._stopAudioTracks();
        this._audioTrackPlayers = [];

        const tracks: any[] = (this._doc.song as any).audioTracks || [];
        for (const track of tracks) {
            if (!track || track.muted) continue;
            if (typeof track.dataUrl !== "string") continue;

            const audio = new Audio(track.dataUrl);
            audio.preload = "auto";
            audio.volume = track.gain ?? 1;
            // For now, start from the beginning when the song plays.
            audio.currentTime = 0;

            audio.play().catch(err => {
                console.error("Failed to play audio track:", track.name, err);
            });

            this._audioTrackPlayers.push(audio);
        }
    }

    // NEW: stop audio tracks when the song pauses.
    private _stopAudioTracks(): void {
        for (const audio of this._audioTrackPlayers) {
            try {
                audio.pause();
            } catch {
                // ignore
