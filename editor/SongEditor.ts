...
class Slider {
	...
	private _setFromPointer(event: PointerEvent): void {
		...
		const thumbWidth: number = 6; // MyBox slider thumbs are styled with a width of 6 pixels.
...
export class SongEditor {
	...
	public readonly mainLayer: HTMLDivElement = div({class: "myboxEditor", tabIndex: "0"},
		this._patternArea,
		this._trackArea,
		this._settingsArea,
	...
	constructor(myboxEditorContainer: HTMLElement) {
		...
		myboxEditorContainer.appendChild(this.mainLayer);
		this.whenUpdated();
		...
		// MyBox uses browser history state as its own undo history. Browsers typically
		// remember scroll position for each history state, but MyBox users would prefer not
