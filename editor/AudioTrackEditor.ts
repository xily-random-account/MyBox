...
export class AudioTrackEditor {
	...
	public render(): void {
		...
		for (const track of tracks) {
			...
			const clip: HTMLDivElement = div({class: "audioClip", draggable: "false", style: `position: absolute; left: ${startBar * this._barWidth}px; top: 3px; height: 26px; width: ${clipWidth}px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; box-sizing: border-box; padding: 5px 8px; color: ${ColorConfig.primaryText}; background: linear-gradient(180deg, color-mix(in srgb, ${ColorConfig.linkAccent} 92%, white), ${ColorConfig.linkAccent}); opacity: ${track.muted ? 0.45 : 0.9}; border-radius: 3px; cursor: grab; user-select: none;`}, span(track.name));
			const waveform: HTMLDivElement = div({style: `position: absolute; inset: 0; opacity: 0.28; pointer-events: none; background: repeating-linear-gradient(90deg, transparent 0, transparent 3px, ${ColorConfig.primaryText} 3px, ${ColorConfig.primaryText} 5px, transparent 5px, transparent 9px);`});
