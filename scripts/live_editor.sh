...
# This performs some but not all of the tasks from compile_mybox_editor.sh.
# tsc is used to transpile TypeScript files to JavaScript files in build/, and
# rollup is used to bundle these files into website/mybox_editor.js. Here,
...
npx concurrently \
	...
	"npx rollup build/editor/index.js --file website/mybox_editor.js --format iife --output.name mybox --context exports --sourcemap --plugin rollup-plugin-sourcemaps --plugin @rollup/plugin-node-resolve --watch.buildDelay 200 -w" \
