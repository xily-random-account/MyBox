...
const enum SongTagCode {
	beatCount           = CharCode.a, // added in MyBox URL version 2
	bars                = CharCode.b, // added in MyBox URL version 2
	vibrato             = CharCode.c, // added in MyBox URL version 2, DEPRECATED
	fadeInOut           = CharCode.d, // added in MyBox URL version 3 for transition, switched to fadeInOut in 9
	loopEnd             = CharCode.e, // added in MyBox URL version 2
	eqFilter            = CharCode.f, // added in MyBox URL version 3
	barCount            = CharCode.g, // added in MyBox URL version 3
	unison              = CharCode.h, // added in MyBox URL version 2
	instrumentCount     = CharCode.i, // added in MyBox URL version 3
	patternCount        = CharCode.j, // added in MyBox URL version 3
	key                 = CharCode.k, // added in MyBox URL version 2
	loopStart           = CharCode.l, // added in MyBox URL version 2
	reverb              = CharCode.m, // added in MyBox URL version 5, DEPRECATED
	channelCount        = CharCode.n, // added in MyBox URL version 6
	channelOctave       = CharCode.o, // added in MyBox URL version 3
	patterns            = CharCode.p, // added in MyBox URL version 2
	effects             = CharCode.q, // added in MyBox URL version 7
	rhythm              = CharCode.r, // added in MyBox URL version 2
	scale               = CharCode.s, // added in MyBox URL version 2
	tempo               = CharCode.t, // added in MyBox URL version 2
	preset              = CharCode.u, // added in MyBox URL version 7
	volume              = CharCode.v, // added in MyBox URL version 2
	wave                = CharCode.w, // added in MyBox URL version 2
	supersaw            = CharCode.x, // added in MyBox URL version 9
	filterResonance     = CharCode.y, // added in MyBox URL version 7, DEPRECATED
	drumsetEnvelopes    = CharCode.z, // added in MyBox URL version 7 for filter envelopes, still used for drumset envelopes
	algorithm           = CharCode.A, // added in MyBox URL version 6
	feedbackAmplitude   = CharCode.B, // added in MyBox URL version 6
	chord               = CharCode.C, // added in MyBox URL version 7, DEPRECATED
//	                    = CharCode.D, // added in JummBox URL version 3(?) for detune, DEPRECATED
	envelopes           = CharCode.E, // added in MyBox URL version 6 for FM operator envelopes, repurposed in 9 for general envelopes.
	feedbackType        = CharCode.F, // added in MyBox URL version 6
//	                    = CharCode.G, // added in JummBox URL version 3 for arpeggioSpeed, DEPRECATED
	harmonics           = CharCode.H, // added in MyBox URL version 7
	stringSustain       = CharCode.I, // added in MyBox URL version 9
//	                    = CharCode.J,
//	                    = CharCode.K,
...
//	                    = CharCode.O, // added in JummBox URL version 3(?) for limiterSettings
	operatorAmplitudes  = CharCode.P, // added in MyBox URL version 6
	operatorFrequencies = CharCode.Q, // added in MyBox URL version 6
//	                    = CharCode.R, // added in JummBox URL version 4 for operatorWaves
	spectrum            = CharCode.S, // added in MyBox URL version 7
	startInstrument     = CharCode.T, // added in MyBox URL version 6
//	                    = CharCode.U, // added in JummBox URL version 4(?) for channelNames
	feedbackEnvelope    = CharCode.V, // added in MyBox URL version 6, DEPRECATED
	pulseWidth          = CharCode.W, // added in MyBox URL version 7
...
export class FilterSettings {
	...
	public convertLegacySettings(legacyCutoffSetting: number, legacyResonanceSetting: number, legacyEnv: Envelope): void {
		...
		if (legacyEnv.type == EnvelopeType.none && !resonant && cutoffAtMax) {
			...
			// However, MyBox's original lowpass filters behaved oddly as they
...
// Settings that were available to old versions of MyBox but are no longer available in the
...
export class Instrument {
	...
	public convertLegacySettings(legacySettings: LegacySettings): void {
		...
		// The "punch" envelope is special: it goes *above* the chosen cutoff. But if the cutoff was already at the max, it couldn't go any higher... except in the current version of MyBox I raised the max cutoff so it *can* but then it sounds different, so to preserve the original sound let's just remove the punch envelope.
		const legacyFilterCutoffRange: number = 11;
		const cutoffAtMax: boolean = (legacyCutoffSetting == legacyFilterCutoffRange - 1);
...
export class Song {
	...
	public fromBase64String(compressed: string): void {
		...
		if (beforeNine) {
			// Unfortunately, old versions of MyBox had a variety of different ways of saving
		...
		while (charIndex < compressed.length) switch(command = compressed.charCodeAt(charIndex++)) {
			...
			case SongTagCode.effects: {
				...
				if (beforeNine) {
					...
					// MyBox currently uses two base64 characters at 6 bits each for a bitfield representing all the enabled effects.
...
// When compiling synth.ts as a standalone module named "mybox", expose these imported classes as members to JavaScript:
export {
	Dictionary,
	DictionaryArray,
	FilterType,
